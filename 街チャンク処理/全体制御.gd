...
func _ready() -> void:
	...
	if データロガー.プレイヤーステート取得(データロガー.プレイヤーデータ.ムービーシーン,"")!="":
		シナリオ演出実行(データロガー.プレイヤーステート取得(データロガー.プレイヤーデータ.ムービーシーン,""))
...
func NPC制御返し()->スケジュール管理クラス:
	return $"NPC制御"

func 演出中()->bool:
	return !$"演出ルート".get_children().is_empty()
