...
func ミッションフラグ追加(ミッションオブジェクト:ミッションデータ)->void:
	...
	辞書.set("サブ",ミッションオブジェクト.サブ)
	辞書.set("優先度",ミッションオブジェクト.優先度)
	辞書.set("目的地",ミッションオブジェクト.目的地)
...
func ミッションフラグあるか(フラグ名:String)->bool:
	return config.has_section_key("ミッションフラグ",フラグ名)
