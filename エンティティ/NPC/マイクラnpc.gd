	...
	while NPC管理 and レベル制御:
		...
		#同じディメンションにいる場合↓
			...
			if 辞書["ディメンション"]!="" and 辞書["ディメンションオブジェクト番号"] and レベル制御.レベルルート:
				...
				if レベル制御.レベルルート.NPC用オブジェクト.size() > 辞書["ディメンションオブジェクト番号"] and レベル制御.レベルルート.NPC用オブジェクト[辞書["ディメンションオブジェクト番号"]]:
					レベル制御.レベルルート.NPC用オブジェクト[辞書["ディメンションオブジェクト番号"]].実行(self)
					到着=true#到着はさせておく
				elif not レベル制御.レベルルート.NPC用オブジェクト.size() > 辞書["ディメンションオブジェクト番号"]:
					printerr("レベル側で設定したNPC用オブジェクトが存在しません。\nアクセスインデクス:",辞書["ディメンションオブジェクト番号"])
					breakpoint
##プレイヤーは引数の方向を向く
