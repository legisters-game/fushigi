...
func _process(delta: float) -> void:
	...
	var force_y = sin(time * wave_speed) * wave_intensity
	
	var force_x = cos(time * wave_speed * 0.7) * (wave_intensity * 0.5) # 少し複雑な揺れにする
	if 反転:
		force_x=-force_x
	
	# 常に変化する力を与え続けることで、強制的にボーンを往復運動（ゆらゆら）させる
	active= (アニメーションプレイヤー and not アニメーションプレイヤー.current_animation)
	print(アニメーションプレイヤー.current_animation)
	
	
	if 三次元:
		var force_z = -cos(time * wave_speed * 0.9) * (wave_intensity * 0.7)
